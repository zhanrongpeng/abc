...
static void Abc_NtkMarkTfiPoName_rec( Abc_Obj_t * pObj, const char * pPoName, void ** vMap, int * vBestDepth, int Depth );
...
static void Abc_NtkMarkTfiPoName_rec( Abc_Obj_t * pObj, const char * pPoName, void ** vMap, int * vBestDepth, int Depth )
{
    ...
    {
        Abc_NtkMarkTfiPoName_rec( Abc_ObjFanin0(pObj), pPoName, vMap, vBestDepth, Depth + 1 );
        Abc_NtkMarkTfiPoName_rec( Abc_ObjFanin1(pObj), pPoName, vMap, vBestDepth, Depth + 1 );
    }
}
...
If_Man_t * Abc_NtkToIf( Abc_Ntk_t * pNtk, If_Par_t * pPars )
{
    ...
    {
        int * vBestDepth;
        pIfMan->nNodeMapSize = Abc_NtkObjNumMax(pNtk) + 4;
        pIfMan->vNodeNameMap = (void **)ABC_CALLOC( void *, pIfMan->nNodeMapSize );
        // Allocate original-node -> representative PO/net name map.
        pIfMan->vNodeDrivingPoName = (void **)ABC_CALLOC( void *, pIfMan->nNodeMapSize );
        // Allocate If node -> output-net name map.
        pIfMan->vNodeOutputNetName = (void **)ABC_CALLOC( void *, pIfMan->nNodeMapSize );
        vBestDepth = ABC_ALLOC( int, pIfMan->nNodeMapSize );
        for ( i = 0; i < pIfMan->nNodeMapSize; i++ )
            vBestDepth[i] = 0x3fffffff;
        // Propagate each PO name to all nodes in its TFI cone.
        // This provides a stable coordinate lookup key for both CI and AND nodes.
        Abc_NtkForEachCo( pNtk, pNode, i )
        {
            Abc_Obj_t * pFanin = Abc_ObjFanin0( pNode );
            const char * poName = Abc_ObjName( pNode );
            Abc_NtkMarkTfiPoName_rec( pFanin, poName, pIfMan->vNodeDrivingPoName, vBestDepth, 0 );
        }
        ABC_FREE( vBestDepth );
    }
    ...
    Abc_NtkForEachCi( pNtk, pNode, i )
    {
        ...
        if ( pIfMan->vNodeOutputNetName )
        {
            const char * pOutName = NULL;
            if ( pIfMan->vNodeDrivingPoName )
                pOutName = (const char *)pIfMan->vNodeDrivingPoName[Abc_ObjId(pNode)];
            if ( pOutName == NULL || pOutName[0] == '\0' )
                pOutName = Abc_ObjName( pNode );
            pIfMan->vNodeOutputNetName[ If_ObjId(pIfObj) ] = (void *)Extra_UtilStrsav( pOutName );
        }
    }
    ...
    Vec_PtrForEachEntry( Abc_Obj_t *, vNodes, pNode, i )
    {
        ...
        if ( pIfMan->vNodeOutputNetName )
        {
            ...
            if ( pOutName == NULL || pOutName[0] == '\0' )
                pOutName = Abc_ObjName( pNode );
            pIfMan->vNodeOutputNetName[ If_ObjId((If_Obj_t *)pNode->pCopy) ] = (void *)Extra_UtilStrsav( pOutName );
        }
