...
Abc_Ntk_t * Abc_NtkMap( Abc_Ntk_t * pNtk, Mio_Library_t* userLib, double DelayTarget, double AreaMulti, double DelayMulti, float LogFan, float Slew, float Gain, int nGatesMin, int fRecovery, int fSwitching, int fSkipFanout, int fUseProfile, int fUseBuffs, int fVerbose )
{
    ...
    if ( pMan == NULL )
        return NULL;
    // wire-aware mapping: read coordinates and RC from ABC frame
    Map_ManSetWireAwareFromFrame( pMan );
    if ( pMan->WireDelay > 0.0f && pMan->pNtkCoords )
    {
        printf( "Wire-aware map enabled: WireR=%.4f ohm/um  WireC=%.4f fF/um\n",
                pMan->WireR, pMan->WireC );
        // Debug: check vNodeNameMap was populated
        int node_count = 0;
        for ( int dbg_i = 0; dbg_i < pMan->nNodeMapSize; dbg_i++ )
            if ( pMan->vNodeNameMap[dbg_i] != NULL )
                node_count++;
        printf( "  [DEBUG] vNodeNameMap: %d / %d entries populated\n", node_count, pMan->nNodeMapSize );
    }
clk = Abc_Clock();
...
/**Function*************************************************************
  ...
  Synopsis    [Recursively propagate PO name to all TFI nodes.]

  Description [Given a node, assign it a "driving PO name" — the net name
    of the PO this node ultimately drives. Used for wire-aware mapping to
    look up the correct net coordinates from the coords file (which stores
    output net names, not internal node names).]
...
***********************************************************************/
static void Abc_MapMarkTfiPoName_rec( Abc_Obj_t * pObj, const char * pPoName, void ** vMap, int * vBestDepth, int Depth )
{
    if ( pObj == NULL || pPoName == NULL || vMap == NULL )
        return;
    if ( !Abc_ObjIsNode(pObj) && !Abc_ObjIsCi(pObj) )
        return;
    if ( vBestDepth && vBestDepth[Abc_ObjId(pObj)] <= Depth )
        return;
    if ( vBestDepth )
        vBestDepth[Abc_ObjId(pObj)] = Depth;
    if ( vMap[Abc_ObjId(pObj)] != NULL )
        ABC_FREE( vMap[Abc_ObjId(pObj)] );
    vMap[Abc_ObjId(pObj)] = (void *)Extra_UtilStrsav( pPoName );
    if ( Abc_ObjIsNode(pObj) )
    {
        Abc_MapMarkTfiPoName_rec( Abc_ObjFanin0(pObj), pPoName, vMap, vBestDepth, Depth + 1 );
        Abc_MapMarkTfiPoName_rec( Abc_ObjFanin1(pObj), pPoName, vMap, vBestDepth, Depth + 1 );
    }
}
...
Map_Man_t * Abc_NtkToMap( Abc_Ntk_t * pNtk, double DelayTarget, int fRecovery, float * pSwitching, int fVerbose )
{
    ...
    // Wire-aware mapping: allocate name maps.
    {
        int nMax = Abc_NtkObjNumMax(pNtk);
        pMan->nNodeMapSize = nMax + 4;
        pMan->vNodeNameMap = ABC_CALLOC( void *, pMan->nNodeMapSize );
        pMan->vNodeOutputNetName = ABC_CALLOC( void *, pMan->nNodeMapSize );
    }

    // ----- Wire-aware: propagate PO output-net names to all TFI nodes -----
    // This stores the "driving PO name" for each node, which is the net name
    // stored in the coords file (e.g. "_000_") instead of the internal node
    // name (e.g. "gcd.u4"). The shortest PO path is recorded (BFS from PO).
    {
        int nMax = Abc_NtkObjNumMax(pNtk);
        int * vBestDepth = ABC_ALLOC( int, nMax + 4 );
        for ( i = 0; i < nMax + 4; i++ )
            vBestDepth[i] = 0x3fffffff;
        Abc_NtkForEachCo( pNtk, pNode, i )
        {
            Abc_Obj_t * pFanin = Abc_ObjFanin0( pNode );
            const char * poName = Abc_ObjName( pNode );
            Abc_MapMarkTfiPoName_rec( pFanin, poName, pMan->vNodeDrivingPoName, vBestDepth, 0 );
        }
        ABC_FREE( vBestDepth );
    }
    ...
    Abc_NtkForEachCi( pNtk, pNode, i )
    {
        ...
        // wire-aware: store original BLIF node pointer and driving PO net name
        Map_NodeSetOrigNode( pNodeMap, pNode );
        // wire-aware: store driving PO output-net name for coordinate lookup
        if ( pMan->vNodeOutputNetName && pNode->Id < pMan->nNodeMapSize )
        {
            const char * pPoName = (const char *)pMan->vNodeNameMap[pNode->Id];
            if ( pPoName && pPoName[0] != '\0' )
                pMan->vNodeOutputNetName[pNode->Id] = (void *)Extra_UtilStrsav( pPoName );
            else
                pMan->vNodeOutputNetName[pNode->Id] = (void *)Extra_UtilStrsav( Abc_ObjName(pNode) );
        }
    }
