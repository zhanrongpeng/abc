...
static int Io_ReadCoordsNetwork( Io_ReadCoords_t * p )
{
    ...
    while ( fgets(sBuffer, 1000, p->pFile) != NULL )
    {
        ...
        if ( sBuffer[0] == '#' )
        {
            // Check for wire_rc directive: "#wire_rc R C" or "# wire_rc R C"
            float r_val = 0.0f, c_val = 0.0f;
            // Find "wire_rc" in the buffer, then parse the two floats after it.
            // This handles both "#wire_rc" (no space) and "# wire_rc" (with space).
            const char * pWireRc = strstr( sBuffer, "wire_rc" );
            if ( pWireRc != NULL && sscanf( pWireRc + 7, "%f %f", &r_val, &c_val ) == 2 )
            {
