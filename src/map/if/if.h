...
struct If_Man_t_
{
    ...
    // wire-aware mapping: If_ObjId -> blif node pointer (for debug/fallback lookup)
    void **            vNodeNameMap;  // void** array, allocated by ABC_CALLOC
    // wire-aware mapping: original Abc_ObjId -> representative PO/net name
    // (computed on original network before If graph creation)
    void **            vNodeDrivingPoName;  // void** array, allocated by ABC_CALLOC
    // wire-aware mapping: If_ObjId -> output net name used for coordinate lookup
    // (stable string copy, owned/freed by If_Man)
    void **            vNodeOutputNetName;  // void** array, allocated by ABC_CALLOC
    int                nNodeMapSize;   // allocated size of node-name maps
