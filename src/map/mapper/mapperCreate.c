...
//    Map_NodeVecFree( p->pConst1->vFanouts );
    ...
    // wire-aware mapping name map arrays
    // vNodeNameMap: stores Abc_Obj_t* pointers (not owned)
    if ( p->vNodeNameMap )
    {
        ABC_FREE( p->vNodeNameMap );
    }
    // vNodeDrivingPoName: stores allocated strings (owned)
    if ( p->vNodeDrivingPoName )
    {
        int i;
        for ( i = 0; i < p->nNodeMapSize; i++ )
            ABC_FREE( p->vNodeDrivingPoName[i] );
        ABC_FREE( p->vNodeDrivingPoName );
    }
    if ( p->vNodeOutputNetName )
    {
        int i;
        for ( i = 0; i < p->nNodeMapSize; i++ )
            ABC_FREE( p->vNodeOutputNetName[i] );
        ABC_FREE( p->vNodeOutputNetName );
    }
    ABC_FREE( p->pInputs );
    ABC_FREE( p->pOutputs );
    ABC_FREE( p->pBins );
...
////////////////////////////////////////////////////////////////////////
///  WIRE-AWARE MAPPING API (Elmore delay from cut centroid positions)
////////////////////////////////////////////////////////////////////////

/**Function*************************************************************

  Synopsis    [Set wire-aware mapping parameters from the ABC frame.]

  Description [Called by Abc_NtkMap before Map_Mapping.
  Reads pNtkCoords and wire RC from the ABC global frame.
  Builds vNodeNameMap from pAigNodeIDs.
  WireDelay > 0 enables wire-aware mapping.]
...
***********************************************************************/
void Map_ManSetWireAwareFromFrame( Map_Man_t * p )
{
    Abc_Frame_t * pAbc = Abc_FrameGetGlobalFrame();
    if ( pAbc == NULL )
        return;
    // get coordinate data from ABC frame (void* = Io_ReadCoords_t*)
    extern void * Io_ReadCoordsGetFromFrame( Abc_Frame_t * pAbc );
    p->pNtkCoords = Io_ReadCoordsGetFromFrame( pAbc );
    // get wire RC from ABC frame
...
/**Function*************************************************************
  ...
  Synopsis    [Returns the LValue (accumulated wirelength in um) of a node.]

  Description [Used by Map_TimeCutComputeArrival to get the wirelength
  from this node's output to the next stage.]
...
***********************************************************************/
float Map_NodeReadLValue( Map_Node_t * pNode )
{
    return pNode->LValue;
}
...
/**Function*************************************************************
  ...
  Synopsis    [Sets the LValue (accumulated wirelength in um) of a node.]

  Description [Called during forward delay propagation in wire-aware mapping.]
...
***********************************************************************/
void Map_NodeSetLValue( Map_Node_t * pNode, float LValue )
{
    pNode->LValue = LValue;
}
...
/**Function*************************************************************
  ...
  Synopsis    [Get x coordinate of a node by its AIG ID.]

  Description [Looks up the original BLIF node pointer from vNodeNameMap,
  then queries pNtkCoords for the (x,y) position.]
...
***********************************************************************/
float Map_NodeReadCoordX( Map_Man_t * p, int AigId )
{
    if ( p->pNtkCoords == NULL || p->vNodeNameMap == NULL || AigId < 0 || AigId >= p->nNodeMapSize )
        return 0.0f;
    void * pBlifNodePtr = p->vNodeNameMap[AigId];
    if ( pBlifNodePtr == NULL )
        return 0.0f;
    Abc_Obj_t * pBlifNode = (Abc_Obj_t *)pBlifNodePtr;
    float x, y;
    if ( Io_ReadCoordsGetCoordByName( p->pNtkCoords, Abc_ObjName(pBlifNode), &x, &y ) )
        return x;
    return 0.0f;
}
float Map_NodeReadCoordY( Map_Man_t * p, int AigId )
{
    if ( p->pNtkCoords == NULL || p->vNodeNameMap == NULL || AigId < 0 || AigId >= p->nNodeMapSize )
        return 0.0f;
    void * pBlifNodePtr = p->vNodeNameMap[AigId];
...
/**Function*************************************************************
  ...
  Synopsis    [Store the BLIF node pointer for a given AIG ID.]

  Description [Used by Abc_NtkToMap to populate vNodeNameMap.]
...
***********************************************************************/
void Map_ManSetNodeName( Map_Man_t * p, int AigId, void * pBlifNode )
{
    if ( p->vNodeNameMap == NULL || AigId < 0 || AigId >= p->nNodeMapSize )
        return;
    p->vNodeNameMap[AigId] = pBlifNode;
}
void Map_NodeSetOrigNode( Map_Node_t * pNode, void * pOrigNode )
{
    pNode->pOrigNode = pOrigNode;
}
void * Map_NodeReadOrigNode( Map_Node_t * pNode )
{
    return pNode->pOrigNode;
}
const char * Map_NodeReadDrivingPoName( Map_Man_t * p, Map_Node_t * pNode )
{
    if ( p->vNodeDrivingPoName == NULL )
        return NULL;
    int aigId = Map_NodeReadAigId( pNode );
    if ( aigId < 0 || aigId >= p->nNodeMapSize )
        return NULL;
    return (const char *)p->vNodeDrivingPoName[aigId];
}
