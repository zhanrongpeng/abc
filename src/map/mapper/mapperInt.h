...
struct Map_ManStruct_t_
{
    ...
    // wire-aware mapping: name -> coordinate lookup
    void *              pNtkCoords;         // Io_ReadCoords_t* from read_coords
    float               WireDelay;           // wire delay coefficient (0 = disabled)
    float               WireR;               // ohm/um
    float               WireC;               // fF/um
    // Map_Node->Num -> BLIF node pointer (Abc_Obj_t*) for pOrigNode
    void **             vNodeNameMap;
    // Map_Node->Num -> original node driving PO name (char*, stable) for coord lookup
    void **             vNodeDrivingPoName;
    // Map_Node->Num -> representative output net name (char*, stable)
    void **             vNodeOutputNetName;
    int                 nNodeMapSize;       // allocated size of the above arrays
...
struct Map_NodeStruct_t_ 
{
    ...
    float               LValue;        // accumulated wirelength of cut output (um, wire-aware)
    void *              pOrigNode;     // original Abc_Obj_t* for wire-aware name lookup
