...
float Map_TimeCutComputeArrival( Map_Node_t * pNode, Map_Cut_t * pCut, int fPhase, float tWorstLimit )
{
    ...
    Map_Time_t * ptArrIn;
    int fPinPhase;
    float tDelay, tExtra, tWireDelay;
    int i;

    // fanout-logic delay (LogFan-based)
    tExtra = p->pNodeDelays ? p->pNodeDelays[pNode->Num] : 0;

    // ================================================================
    // Wire-aware mapping: compute Elmore wire delay from cut centroid
    //
    // Model: cut output wire starts at centroid of fanin cell positions.
    //   wirelength = max over leaves: (leaf.LValue + dist(leaf_pos, centroid))
    //   wire_delay = R * C * L^2 / 2
    //   tWireDelay added to tExtra (cell + wire combined delay)
    // ================================================================
    tWireDelay = 0.0f;
    if ( p->WireDelay > 0.0f && p->pNtkCoords )
    {
        ...
        if ( n_fanins > 0 )
        {
            ...
            // Phase 3: propagate LValue to this node for parent nodes
            Map_NodeSetLValue( pNode, worst_wl );

            // Phase 4: compute Elmore wire delay (ps)
            // wire_delay = R(ohm/um) * C(fF/um) * L(um)^2 / 2 * 1e-3
            tWireDelay = p->WireR * p->WireC * worst_wl * worst_wl * 0.5f * 1e-3f;
        }
    }
    ...
    for ( i = pCut->nLeaves - 1; i >= 0; i-- )
    {
        ...
        {
            tDelay = ptArrIn->Rise + pSuper->tDelaysR[i].Rise + tExtra + tWireDelay;
        ...
        {
            tDelay = ptArrIn->Fall + pSuper->tDelaysR[i].Fall + tExtra + tWireDelay;
            if ( tDelay > tWorstLimit )
                return MAP_FLOAT_LARGE;
            if ( ptArrRes->Rise < tDelay )
                ptArrRes->Rise = tDelay;
        ...
        {
            tDelay = ptArrIn->Rise + pSuper->tDelaysF[i].Rise + tExtra + tWireDelay;
            if ( tDelay > tWorstLimit )
                return MAP_FLOAT_LARGE;
        ...
        {
            tDelay = ptArrIn->Fall + pSuper->tDelaysF[i].Fall + tExtra + tWireDelay;
            if ( tDelay > tWorstLimit )
                return MAP_FLOAT_LARGE;
            if ( ptArrRes->Fall < tDelay )
                ptArrRes->Fall = tDelay;
